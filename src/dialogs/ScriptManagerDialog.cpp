    ...
    : QDialog(parent)
{
    ...
    connect(
        ...
        &QToolButton::clicked,
        this,
        &ScriptManagerDialog::openScriptsDirectory);
    ...
    connect(newButton, &QPushButton::clicked, this, &ScriptManagerDialog::newScript);
    connect(openButton, &QPushButton::clicked, this, &ScriptManagerDialog::openScript);
    connect(saveButton, &QPushButton::clicked, this, &ScriptManagerDialog::saveScript);
    connect(saveAsButton, &QPushButton::clicked, this, &ScriptManagerDialog::saveScriptAs);
    ...
    buttons->addButton(openButton, QDialogButtonBox::ActionRole);
    buttons->addButton(saveButton, QDialogButtonBox::ActionRole);
    buttons->addButton(saveAsButton, QDialogButtonBox::ActionRole);
    buttons->addButton(reloadButton, QDialogButtonBox::ActionRole);
    buttons->addButton(runButton, QDialogButtonBox::ActionRole);
    buttons->addButton(stopButton, QDialogButtonBox::ActionRole);
...
void ScriptManagerDialog::refreshScriptList()
{
...
void ScriptManagerDialog::selectScriptFromTree(QTreeWidgetItem *item, int)
{
...
void ScriptManagerDialog::openScript()
{
    if (!maybeSave()) {
        return;
    }
...
void ScriptManagerDialog::saveScript()
{
...
void ScriptManagerDialog::saveScriptAs()
{
...
void ScriptManagerDialog::reloadCurrentScript()
{
    if (currentFilePath.isEmpty()) {
        return;
    }
...
void ScriptManagerDialog::runCurrentScript()
{
    if (runningTask && runningTask->isRunning()) {
        return;
    }
...
void ScriptManagerDialog::stopRunningScript()
{
    if (!runningTask || !runningTask->isRunning()) {
        return;
    }
...
void ScriptManagerDialog::openScriptsDirectory()
{
    QDesktopServices::openUrl(QUrl::fromLocalFile(scriptsDirectoryPath()));
}

bool ScriptManagerDialog::maybeSave()
{
    if (!editor->document()->isModified()) {
        return true;
    }
...
bool ScriptManagerDialog::loadScriptFile(const QString &path)
{
    QFile file(path);
    QString contents;
...
bool ScriptManagerDialog::saveScriptFile(const QString &path)
{
...
void ScriptManagerDialog::addScriptEntry(
...
void ScriptManagerDialog::updateButtonState()
{
...
void ScriptManagerDialog::updateHighlighter()
{
    delete highlighter;
...
void ScriptManagerDialog::selectPathInTree(const QString &path)
{
    if (path.isEmpty()) {
...
QString ScriptManagerDialog::displayNameForPath(const QString &path) const
{
    if (path.isEmpty()) {
...
QString ScriptManagerDialog::scriptsDirectoryPath() const
{
    return Core()->getIaitoRCDefaultDirectory().absolutePath();
}

QString ScriptManagerDialog::defaultSavePath() const
{
    if (!currentFilePath.isEmpty()) {
...
QString ScriptManagerDialog::prepareRunnableScriptFile()
{
