...
ColorPickArea::ColorPickArea(QWidget *parent)
...
QColor ColorPickArea::pointToColor(int x, int y) const
{
    ...
    color.setHsvF(MyRealType(x) / width(), 1.0 - MyRealType(y) / height(), v, a);
    return color;
}
    ...
    , pickingFromScreen(false)
{
    ...
    connect(ui->alphaChannelBar, &AlphaChannelBar::colorChanged, this, [this](const QColor &color) {
...
ColorShowWidget::ColorShowWidget(QWidget *parent)
    : ColorPickWidgetAbstract(parent)
{}
...
void ColorShowWidget::paintEvent(QPaintEvent *event)
{
    ...
    for (int y = rect().topLeft().ry(); y < rect().bottomRight().ry(); y++) {
        for (int x = rect().topLeft().rx(); x < rect().bottomRight().rx(); x++) {
            p.setPen(
                ((x % miniRectWidth) / (miniRectWidth / 2))
