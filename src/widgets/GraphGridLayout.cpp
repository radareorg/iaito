...
# Terms used:
- **Vertex**, **node**, **block** - read description of graph for definition.
Within this text vertex and node are used interchangeably with block due to code
being written for visualizing basic block control flow graph.
- **edge** - read description of graph for definition for precise definition.
- **DAG** - directed acyclic graph, graph using directed edges which doesn't
have cycles. DAG may contain loops if following them would require going in both
directions of edges. Example 1->2 1->3 3->2 is a DAG, 2->1 1->3 3->2 isn't a
DAG.
- **DFS** - depth first search, a graph traversal algorithm
- **toposort** - topological sorting, process of ordering a DAG vertices that
all edges go from vertices earlier in the toposort order to vertices later in
toposort order. There are multiple algorithms for implementing toposort
operation. Single DAG can have multiple valid topological orderings, a toposort
algorithm can be designed to prioritize a specific one from all valid toposort
orders. Example: for graph 1->4, 2->1, 2->3, 3->4 valid topological orders are
[2,1,3,4] and [2,3,1,4].
...
4. assign node positions within grid using tree structure, child subtrees are
placed side by side with parent on top
5. perform edge routing
6. calculate column and row pixel positions based on node sizes and amount edges
between the rows
7. [optional] layout compacting


Contrary to many other layered graph drawing algorithm this implementation
doesn't perform node reordering to minimize edge crossing. This simplifies
implementation, and preserves original control flow structure for conditional
jumps ( true jump on one side, false jump on other). Due to most of control flow
being result of structured programming constructs like if/then/else and loops,
resulting layout is usually readable without node reordering within layers.


# Description of grid.
To simplify the layout algorithm initial steps assume that all nodes have the
same size and edges are zero width. After placing the nodes and routing the
edges it is known which nodes are in in which row and column, how many edges are
between each pair of rows. Using this information positions are converted from
the grid cells to pixel coordinates. Routing 0 width edges between rows can also
be interpreted as every second row and column being reserved for edges. The row
numbers in code are using first interpretation. To allow better centering of
nodes one above other each node is 2 columns wide and 1 row high.

\image html graph_grid.svg

# 1-2 Cycle removal and toposort

Cycle removal and toposort are done at the same time during single DFS
traversal. In case entrypoint is part of a loop DFS started from entrypoint.
This ensures that entrypoint is at the top of resulting layout if possible.
Resulting toposort order is used in many of the following layout steps that
require calculating some property of a vertex based on child property or the
other way around. Using toposort order such operations can be implemented
iteration through array in either forward or reverse direction. To prevent
running out of stack memory when processing large graphs DFS is implemented
non-recursively.

# Row assignment

Rows are assigned in toposort order from top to bottom, with nodes row being
max(predecessor.row)+1. This ensures that loop edges are only ones going from
deeper levels to previous layers.

To further simply node placement a subset of edges is selected which forms a
tree. This turns DAG drawing problem into a tree drawing problem. For each node
in level n following nodes which have level exactly n+1 are greedily assigned as
child nodes in tree. If a node already has parent assigned then corresponding
edge is not part of tree.

# Node position assignment

Since the graph has been reduced to a tree, node placement is more or less
putting subtrees side by side with parent on top. There is some room for
interpretation what exactly side by side means and where exactly on top is.
Drawing the graph either too dense or too big may make it less readable so there
are configuration options which allow choosing these things resulting in more or
less dense layout.

Once the subtrees are placed side by side. Parent node can be placed either in
the middle of horizontal bounds or in the middle of direct children. First
option results in narrower layout and more vertical columns. Second option
results in nodes being more spread out which may help seeing where each edge
goes.

In more compact mode two subtrees are placed side by side taking into account
their shape. In wider mode bounding box of shorter subtree is used instead of
exact shape. This gives slightly sparse layout without it being too wide.

\image html graph_parent_placement.svg

# Edge routing
Edge routing can be split into: main column selection, rough routing, segment
offset calculation.

Transition from source to target row is done using single vertical segment. This
is called main column.

Rough routing creates the path of edge using up to 5 segments using grid
coordinates. Due to nodes being placed in a grid. Horizontal segments of edges
can't intersect with any nodes. The path for edges is chosen so that it consists
of at most 5 segments, typically resulting in sideways U shape or square Z
shape.
- short vertical segment from node to horizontal line
- move to empty column
- vertical segment between starting row and end row, an empty column can always
be found, in the worst case there are empty columns at the sides of drawing
- horizontal segment to target node column
- short vertical segment connecting to target node

There are 3 special cases:
- source and target nodes are in the same column with no nodes between - single
vertical segment
- column bellow stating node is empty - segments 1-3 are merged
- column above target node is empty - segments 3-5 are merged
Vertical segment intersection with nodes is prevented using a 2d array marking
which vertical segments are blocked and naively iterating through all rows
between start and end at the desired column.

After rough routing segment offsets are calculated relative to their
corresponding edge column. This ensures that two segments don't overlap. Segment
offsets within each column are assigned greedily with some heuristics for
assignment order to reduce amount of edge crossings and result in more visually
pleasing output for a typical CFG graph. Each segment gets assigned an offset
that is maximum of previously assigned offsets overlapping with current segment
+ segment spacing. Assignment order is chosen based on:
* direction of previous and last segment - helps reducing crossings and place
the segments between nodes
* segment length - reduces crossing when segment endpoints have the same
structure as valid parentheses expression
* edge length - establishes some kind of order when single node is connected to
many edges, typically a block with switch statement or block after switch
statement.

# Layout compacting

Doing the layout within a grid causes minimal spacing to be limited by widest
and tallest block within each column and row. One common case is block with
function entrypoint being wider due to function name causing wide horizontal
space between branching blocks. Another case is rows in two parallel columns
being aligned.

\image html layout_compacting.svg

Both problems are mitigated by squishing graph. Compressing in each of the two
direction is done separately. The process is defined as liner program. Each
variable represents a position of edge segment or node in the direction being
optimized.

Following constraints are used
- Keep the order with nearest segments.
- If the node has two outgoing edges, one to the node on left side and other to
the right, keep them on the corresponding side of node's center.
- For all edges keep the node which is above above. This helps when vertical
block spacing is set bigger than double edge spacing and edge shadows
relationship between two blocks.
- Equality constraint to keep relative position between nodes and and segments
directly connected to them.
- Equality constraint to keep the node centered when control flow merges
In the vertical direction objective function minimizes y positions of nodes and
lengths of vertical segments. In the horizontal direction objective function
minimizes lengths of horizontal segments.

In the resulting linear program all constraints beside x_i >= 0 consist of
exactly two variables: either x_i - x_j <= c_k or x_i = x_j + c_k.

Since it isn't necessary get perfect solution and to avoid worst case
performance current implementation isn't using a general purpose linear
programming solver. Each variable is changed until constraint is reached and
afterwards variables are grouped and changed together.
...
void GraphGridLayout::assignRows(
    GraphGridLayout::LayoutState &state, const std::vector<ut64> &blockOrder)
{
...
void GraphGridLayout::CalculateLayout(
    GraphLayout::Graph &blocks, ut64 entry, int &width, int &height) const
{
...
void GraphGridLayout::computeAllBlockPlacement(
    const std::vector<ut64> &blockOrder, LayoutState &layoutState) const
{
    assignRows(layoutState, blockOrder);
    selectTree(layoutState);
    findMergePoints(layoutState);

    // Shapes of subtrees are maintained using linked lists. Each value within
    // list is column relative to previous row. This allows moving things around
    // by changing only first value in list.
    LinkedListPool<int> sides(blockOrder.size() * 2); // *2 = two sides for each node

    // Process nodes in the order from bottom to top. Ensures that all subtrees
    // are processed before parent node.
    for (auto blockId : blockOrder) {
        ...
        if (block.tree_edge.size() == 0) {
            ...
            auto leftSide = firstChild.leftSideShape; // left side of block children
            ...
            for (size_t i = 1; i < block.tree_edge.size(); i++) {
                ...
                while (leftIt && rightIt) { // process part of subtrees that
                                            // touch when put side by side
                    leftPos += *leftIt;
                    rightPos += *rightIt;
                ...
                if (tightSubtreePlacement) {
                    ...
                    // mode b) use bounding box for shortest subtree and full
                    // shape of other side
                    if (leftIt) {
                ...
                // Calculate the new shape after putting the two subtrees side
                // by side
            ...
            if (parentBetweenDirectChild) {
                ...
                // mode b) somewhere between left most direct child and right
                // most, preferably in the middle of horizontal dimensions.
                // Results layout looks more like single vertical line.
            ...
            block.col += col; // += instead of = to keep offset calculated in
                              // previous steps
            block.row_count += 1;
            ...
            *sides.head(rightSide) -= block.col + 2;
            ...
            // Keep children positions relative to parent so that moving parent
            // moves whole subtree
            for (auto target : block.tree_edge) {
    ...
    // Calculate root positions. Typical function should have one root node that
    // matches with entrypoint. There can be more of them in case of switch
    // statement analysis failure, unreahable basic blocks or using the
    // algorithm for non control flow graphs.
    ...
    int minColumn = 0;
    for (auto &blockIt : layoutState.grid_blocks) {
        minColumn = std::min(minColumn, blockIt.second.col);
    }
    if (minColumn < 0) {
        for (auto &blockIt : layoutState.grid_blocks) {
            blockIt.second.col -= minColumn;
        }
    }
}
...
void GraphGridLayout::calculateEdgeMainColumn(GraphGridLayout::LayoutState &state) const
{
    ...
    // Use sweep line approach processing events sorted by row top to bottom.
    // Use an appropriate tree structure to contain blocks above sweep line and
    // query for nearest column which isn't blocked by a block.
    ...
    for (const auto &event : events) {
        if (event.type == Event::Block) {
            ...
            // Prefer using the same column as starting node, it allows reducing
            // amount of segments.
		...
		if (nearestLeft == -1 || nearestRight == -1) {
                ...
                if (targetBlock.row < block.row) {
                    if (targetColumn < column && blockedColumns.valueAtPoint(column + 1) < topRow
                        ...
                        targetColumn > column && blockedColumns.valueAtPoint(column - 1) < topRow
                ...
                if (distanceLeft != distanceRight) {
                    ...
                    // In case of tie choose based on edge index. Should result
                    // in true branches being mostly on one side, false branches
                    // on other side.
    ...
    const int maxColumn = std::max(0, state.columns);
    for (auto &edgeList : state.edge) {
        const auto &startBlock = state.grid_blocks[edgeList.first];
        for (auto &edge : edgeList.second) {
            if (edge.mainColumn < 0) {
                edge.mainColumn = startBlock.col + 1;
            }
            edge.mainColumn = std::clamp(edge.mainColumn, 0, maxColumn);
        }
    }
}
...
void GraphGridLayout::roughRouting(GraphGridLayout::LayoutState &state) const
{
    ...
    for (auto &blockIt : state.grid_blocks) {
        ...
        for (size_t i = 0; i < blockEdges.size(); i++) {
            ...
            int mainColumn = std::clamp(edge.mainColumn, 0, maxColumn);
            edge.mainColumn = mainColumn;
            ...
            if (mainColumn < start.col + 1 && mainColumn < target.col + 1) {
                mainColumnKind = +2;
            } else if (mainColumn > start.col + 1 && mainColumn > target.col + 1) {
                mainColumnKind = -2;
            } else if (mainColumn == start.col + 1 && mainColumn != target.col + 1) {
                mainColumnKind = mainColumn < target.col + 1 ? 1 : -1;
            } else if (mainColumn == target.col + 1 && mainColumn != start.col + 1) {
                mainColumnKind = mainColumn < start.col + 1 ? 1 : -1;
            }
...
namespace {
/**
 * @brief Single segment of an edge. An edge can be drawn using multiple
 * horizontal and vertical segments. x y meaning matches vertical segments. For
 * horizontal segments axis are swapped.
 */
struct EdgeSegment
{
    int y0;
    ...
    int16_t kind;
    int16_t spacingOverride; //< segment spacing override, 0 if default spacing
                             //should be used
};
struct NodeSide
{
 ...
 * @param edgeOffsets Output argument for returning segment offsets relative to
 * their columns.
 * @param edgeColumnWidth InOut argument describing how much column with edges
 * take. Initial value used as minimal value. May be increased to depending on
 * amount of segments in each column and how tightly they are packed.
 * @param nodeRightSide Right side of nodes. Used to reduce space reserved for
 * edges by placing them between nodes.
 * @param nodeLeftSide Same as right side.
 * @param columnWidth
 * @param H All the segmement and node coordinates y0 and y1 are expected to be
 * in range [0;H)
 * @param segmentSpacing The expected spacing between two segments in the same
 * column. Actual spacing may be smaller for nodes with many edges.
 */
void calculateSegmentOffsets(
    ...
    int segmentSpacing)
{
    if (segments.empty() || edgeOffsets.empty() || edgeColumnWidth.empty() || H == 0) {
        return;
    }

    const int maxX = static_cast<int>(edgeColumnWidth.size()) - 1;
    const int maxY = static_cast<int>(H) - 1;
    for (auto &segment : segments) {
        if (segment.y0 > segment.y1) {
            std::swap(segment.y0, segment.y1);
        }
        segment.x = std::clamp(segment.x, 0, maxX);
        segment.y0 = std::clamp(segment.y0, 0, maxY);
        segment.y1 = std::clamp(segment.y1, 0, maxY);
    }
    for (auto &side : nodeRightSide) {
        side.x = std::clamp(side.x, 0, maxX);
        side.y0 = std::clamp(side.y0, 0, maxY);
        side.y1 = std::clamp(side.y1, 0, maxY);
    }
    for (auto &side : nodeLeftSide) {
        side.x = std::clamp(side.x, 0, maxX);
        side.y0 = std::clamp(side.y0, 0, maxY);
        side.y1 = std::clamp(side.y1, 0, maxY);
    }
...
{
    /* Split segments in each edge column into non intersecting chunks. Center
     * each chunk separately.
     *
     * Process segment endpoints sorted by x and y. Maintain count of currently
     * started segments. When number of active segments reaches 0 there is empty
    ...
    for (const auto &segment : segments) {
        ...
        // Exclude segments which are outside edge column and between the
        // blocks. It's hard to ensure that moving them doesn't cause overlap
        // with blocks.
    ...
    std::sort(events.begin(), events.end(), [](const Event &a, const Event &b) {
        ...
        if (a.y != b.y)
            return a.y < b.y;
        // Process segment start events before end to ensure that
        // activeSegmentCount doesn't go negative and only reaches 0 at the end
        // of chunk.
...
/**
 * @brief Convert segment coordinates from arbitrary range to continuous range
 * starting at 0.
 * @param segments
 * @param leftSides
 * @param rightSides
 ...
 */
static int compressCoordinates(
    std::vector<EdgeSegment> &segments,
...
void GraphGridLayout::elaborateEdgePlacement(GraphGridLayout::LayoutState &state) const
{
    ...
    for (auto &edgeListIt : state.edge) {
        for (const auto &edge : edgeListIt.second) {
            for (size_t j = 1; j < edge.points.size(); j += 2) {
                segments.push_back(segmentFromPoint(
                    edge.points[j],
                    edge,
    ...
    for (auto &blockIt : state.grid_blocks) {
        ...
        // not the same as leftWidth, you would think that one pixel offset
        // isn't visible, but it is
                ...
                for (size_t j = col ? 1 : 2; j < edge.points.size(); j += 2) {
                    ...
                    if (col) {
                        ...
                        if (block) {
                            ...
                            offset = std::min(
                                edgeColumWidth / 2 + std::min(blockWidth, edgeColumWidth) / 2,
                                offset);
                        }
                    }
...
{
    if (state.rows <= 0 || state.columns <= 0 || !state.blocks) {
        qWarning("GraphGridLayout: invalid layout dimensions, skipping width adjustment");
        return;
    }
...
int GraphGridLayout::calculateColumnOffsets(
    const std::vector<int> &columnWidth,
    std::vector<int> &edgeColumnWidth,
...
{
    if (!state.blocks || state.edgeColumnWidth.size() != state.columnWidth.size() + 1
        || state.edgeRowHeight.size() != state.rowHeight.size() + 1) {
    ...
    width = calculateColumnOffsets(
        state.columnWidth, state.edgeColumnWidth, state.columnOffset, state.edgeColumnOffset);
    height = calculateColumnOffsets(
        state.rowHeight, state.edgeRowHeight, state.rowOffset, state.edgeRowOffset);
 ...
 * Changes variables until a constraint is hit, afterwards the two variables are
 * changed together.
 * @param n number of variables
 * @param objectiveFunction coefficients for function \f$\sum c_i x_i\f$ which
 ...
 * @param solution input/output argument, returns results, needs to be
    ...
    bool stickWhenNotMoving)
{
    ...
    // Smallest variable value in the group relative to main one, this is used
    // to maintain implicit x_i >= 0 constraint
    ...
    auto joinSegmentGroups = [&](int a, int b) {
        ...
        // update inequalities and remove some of the constraints between
        // variables that are now grouped
        for (auto it = edgePool.head(edges[b]); it; ++it) {
            ...
            if (getGroup(other) == a) { // skip inequalities where both
                                        // variables are now in the same group
                internalEdgeCount++;
                continue;
            }
            *writeIt++ = *it;
            // Modify the inequalities for the group being attached relative to
            // the main variable in the group to which it is being attached.
    ...
    for (auto &equality : equalities) {
        // process equalities, assumes that initial solution is feasible
        // solution and matches equality constraints
        ...
        if (edgeCount[a] > edgeCount[b]) {
            std::swap(a, b);
            // Update the equality equation so that later variable values can be
            // calculated by simply iterating through them without need to check
            // which direction the group joining was done.
    ...
    // Priority queue for processing groups starting with currently smallest
    // one. Doing it this way should result in number of constraints within
    // group doubling each time two groups are joined. That way each constraint
    // is processed no more than log(n) times.
    std::priority_queue<
        std::pair<int, int>,
    ...
    while (!queue.empty()) {
        ...
        // Find the first constraint which will be hit by changing the variable
        // in the desired direction defined by objective function.
        int limitingGroup = -1;
        int smallestMove = 0;
        if (direction < 0) {
    ...
    std::vector<int> &solution)
{
    ...
    auto uniqueEnd = std::unique(
        inequalities.begin(), inequalities.end(), [](const Constraint &a, const Constraint &b) {
    ...
    for (int i = 0; i < ITERATIONS; i++) {
        ...
        // optimizeLinearProgramPass(n, objectiveFunction, inequalities,
        // equalities, solution, false);
    }
}
...
static Constraint createInequality(
    size_t a, int posA, size_t b, int posB, int minSpacing, const std::vector<int> &positions)
{
...
/**
 * @brief Create inequality constraints from segments which preserves their
 * relative order on single axis.
 *
 * @param segments list of edge segments and block sides
 * @param positions initial element positions before optimization
 * @param blockCount number of variables representing blocks, it is assumed that
 * segments with variableId < \a blockCount represent one side of block.
 * @param variableGroup used to check if segments are part of the same edge and
 * spacing can be reduced
 * @param blockSpacing minimal spacing between blocks
 * @param segmentSpacing minimal spacing between two edge segments, spacing may
 * be less if values in \a positions are closer than this
 * @param inequalities output variable for resulting inequalities, values
 * initially stored in it are not removed
 */
static void createInequalitiesFromSegments(
    ...
    std::vector<Constraint> &inequalities)
{
    // map used as binary search tree y_position -> segment{variableId,
    // x_position} It is used to maintain which segment was last seen in the
    // range y_position..
    ...
    for (auto &segment : segments) {
        ...
        --startPos; // should never be lastSegment.begin() because map is
                    // initialized with segment at pos -1
        ...
        while (it != lastSegments.end() && it->first <= segment.y1) {
            ...
            if (prevSegmentVariable != -1) {
                ...
                inequalities.push_back(createInequality(
                    prevSegmentVariable,
                    prevSegmentPos,
                    segment.variableId,
                    segment.x,
...
void GraphGridLayout::optimizeLayout(GraphGridLayout::LayoutState &state) const
{
    ...
    createInequalitiesFromSegments(
        std::move(segments),
        solution,
        variableGroups,
        blockMapping.size(),
        layoutConfig.blockVerticalSpacing,
        layoutConfig.edgeVerticalSpacing,
        inequalities);
    ...
    createInequalitiesFromSegments(
        ...
        variableGroups,
        blockMapping.size(),
        layoutConfig.blockHorizontalSpacing,
        layoutConfig.edgeHorizontalSpacing,
        inequalities);
    ...
    for (auto &blockIt : *state.blocks) {
        ...
        if (block.edges.size() == 2) {
            ...
            if (blockLeft.x + blockLeft.width < middle && blockRight.x > middle) {
                addInequality(
                    blockMapping[block.edges[0].target],
                    blockLeft.x + blockLeft.width,
                    blockVariable,
                    middle,
                ...
                addInequality(
                    blockVariable,
                    middle,
                    blockMapping[block.edges[1].target],
                    blockRight.x,
                    layoutConfig.blockHorizontalSpacing / 2);
