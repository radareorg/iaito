...
/**
 * @namespace ColorPickerHelpers is a namespace that hides all classes needed
 * for ColorPicker class, because classes inherite QObject can not be declared
 * in *.cpp files or inside of another class.
 */
...
class ColorPickWidgetAbstract : public QWidget
{
    Q_OBJECT
public:
    ColorPickWidgetAbstract(QWidget *parent = nullptr)
        : QWidget(parent)
    {}
 ...
 * from screen or from palette or type in HSV or RGB or HEX representation of
 * color.
 */
...
public:
    ...
    /**
     * @brief isPickingFromScreen returns true if color picker is picking from
     * screen.
     */
...
public slots:
    /**
     * @brief setColor sets displayed color to @a color and emits colorChanged
     * signal.
     */
     ...
     * Function is called automatically when "Pick from screen" button is
     * clicked.
     */
...
private:
    ...
    /**
     * @brief bufferColor is used to buffer current color while picking from
     * screen.
     */
    QColor bufferColor;
};
...
namespace ColorPickerHelpers {
/**
 * @brief The ColorPickerWidget class is parent class for ColorPickArea and
 * ColorValueBar classes.
 */
...
protected:
    ...
    /**
     * @brief pointToColor converts coordinates on widget to color these
     * coordinates represents.
     */
    virtual QColor pointToColor(int x, int y) const = 0;

    /**
     * @brief colorToPoint converts color to coordinates that represent this
     * color.
     */
...
class AlphaChannelBar : public ColorPickerWidget
{
    Q_OBJECT
public:
    AlphaChannelBar(QWidget *parent = nullptr)
...
/**
 * @brief The ColorValueBar class provides widget that helps to set Valuse of
...
class ColorValueBar : public ColorPickerWidget
{
    Q_OBJECT
public:
    ColorValueBar(QWidget *parent = nullptr)
        : ColorPickerWidget(parent)
    {}
