...
/**
 * @brief Graph layout algorithm on layered graph layout approach. For
 * simplicity all the nodes are placed in a grid.
 */
...
private:
    /// false - use bounding box for smallest subtree when placing them side by
    /// side
    bool tightSubtreePlacement = false;
    /// true if code should try to place parent between direct children as much
    /// as possible
    bool parentBetweenDirectChild = false;
    /// false if blocks in rows should be aligned at top, true for middle
    /// alignment
     ...
     * Sets node column offset so that after computing placement merge point is
     * centered bellow nodes above.
     */
     ...
     */
    void computeAllBlockPlacement(
        const std::vector<ut64> &blockOrder, LayoutState &layoutState) const;
    /**
     * @brief Perform the topological sorting of graph nodes.
     * If the graph contains loops, a subset of edges is selected. Subset of
     * edges forming DAG are stored in GridBlock::dag_edge.
     * @param state Graph layout state including the input graph.
     * @param entry Entrypoint node. When removing loops prefer placing this
     * node at top.
    ...
    /**
     * @brief routeEdges Route edges, expects node positions to be calculated
     * previously.
     */
    void routeEdges(LayoutState &state) const;
    /**
     * @brief Choose which column to use for transition from start node row to
     * target node row.
     */
    ...
    /**
     * @brief Recalculate column widths, trying to compensate for the space
     * taken by edge columns.
     */
    void adjustColumnWidths(LayoutState &state) const;
    /**
     * @brief Calculate position of each column(or row) based on widths.
     * It is assumed that columnWidth.size() + 1 = edgeColumnWidth.size() and
     ...
     */
    static int calculateColumnOffsets(
        const std::vector<int> &columnWidth,
        std::vector<int> &edgeColumnWidth,
        std::vector<int> &columnOffset,
        std::vector<int> &edgeColumnOffset);
    /**
     * @brief Final graph layout step. Convert grids cell relative positions to
    ...
    /**
     * @brief Reduce spacing between nodes and edges by pushing everything
     * together ignoring the grid.
     * @param state
     */
