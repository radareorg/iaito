    ...
    , pending(false)
{
    connect(&t, &QTimer::timeout, this, [this]() {
        if (pending) {
            ...
            emit checkComplete(
                QVersionNumber(),
                tr("Time limit exceeded during version check. "
                   "Please check your "
...
void UpdateWorker::showUpdateDialog(bool showDontCheckForUpdatesButton)
{
    ...
    if (ret == QMessageBox::Reset) {
        ...
        QString fullFileName = QFileDialog::getSaveFileName(
            ...
            QStringLiteral("%1").arg(r_file_home(NULL)) + QDir::separator()
        ...
        if (!fullFileName.isEmpty()) {
            ...
            connect(this, &UpdateWorker::downloadFinished, this, [](QString filePath) {
                QMessageBox info(
                    ...
                    tr("Latest version of Iaito was succesfully "
                       "downloaded!"),
...
void UpdateWorker::serveVersionCheckReply()
{
    ...
    if (checkReply->error()) {
        ...
        versionReplyStr
            = QJsonDocument::fromJson(checkReply->readAll()).object().value("tag_name").toString();
...
#endif
    downloadFileName = downloadFileName.arg(latestVersion.toString())
