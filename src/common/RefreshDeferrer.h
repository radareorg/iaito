...
/**
 * @brief Accumulator which simply replaces the current value by an incoming new
 * one
 * @tparam T The type of the param to store
 *
 * This accumulator takes the ownership of all params passed to it and deletes
 * them automatically if not needed anymore!
 */
template<class T>
...
public:
    /**
     * \param Determines whether, if nullptr is passed, the current value should
     * be replaced or kept.
     */
 ...
 *
 * This class can handle the logic necessary to defer the refreshing of widgets
 * when they are not visible. It contains an optional
 * RefreshDeferrerAccumulator, which can be used to accumulate incoming events
 * while refreshing is deferred.
 *
 * Example (don't write it like this in practice, use the convenience methods in
 * IaitoDockWidget):
 * ```
 ...
 *      // Now the RefreshDeferrer tells us to do the refresh and gives us the
 * accumulated param. this->doRefresh(*param);
 * }
 *
 * // ...
 *
 ...
 *          // The RefreshDeferrer takes over the param we passed it in
 * attemptRefresh()
