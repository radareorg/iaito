...
Iaito supports writing plugins in both C++ and Python.
...
If you plan to implement support for a new file format or architecture, Iaito plugins are not the correct approach.
...
which will be created automatically by Iaito.

.. note::
   The support for Python plugins is only available if Iaito was built with the options ``IAITO_ENABLE_PYTHON``
   and ``IAITO_ENABLE_PYTHON_BINDINGS`` enabled.
